...
protected:
    ...
    int boughtItems;
public:
    Customer(const char *_name="Petko", const char *_email="finki@ukim.mk", typeC _type=standard, int _boughtItems=0){
        strcpy(name,_name);
        strcpy(email,_email);
        type=_type;
...
public:
    ...
    FINKI_bookstore& operator+=(const Customer& newCustomer) {
        ...
        newCustomers[numCustomers] = new Customer(newCustomer);
