...
private:
    bool platenSofer;
public:
    AvtomobilTransport(char *_destination="",int _price=0, int _distance=0,bool _platenSofer=true) : Transport(_destination,_price,_distance),platenSofer(_platenSofer){
...
private:
    int passengers;
public:
    KombeTransport(char *_destination="",int _price=0, int _distance=0, int _passengers=0): Transport(_destination,_price,_distance){
...
void pecatiPoloshiPonudi(Transport** pTransport, int n, AvtomobilTransport transport) {
    const double priceThreshold = 1500; // Adjust the threshold as needed
