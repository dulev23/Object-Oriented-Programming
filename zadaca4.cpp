...
private:
    Size size;
public:
    FlatPizza(char*_name="",char *_ingredients="",float _basePrice=0.0, enum Size _type=mala) : Pizza(_name,_ingredients,_basePrice), size(_type){}
...
private:
    bool white;
public:
    FoldedPizza(char*_name="",char *_ingredients="",float _basePrice=0.0, bool _white=true) : Pizza(_name,_ingredients,_basePrice), white(_white){
        white=_white;
    }
...
void expensivePizza(Pizza **pPizza, int n) {
    ...
    if (FlatPizza *fp = dynamic_cast<FlatPizza *>(max)) {
        cout << *fp << endl;
    } else if (FoldedPizza *fp = dynamic_cast<FoldedPizza *>(max)) {
        cout << *fp << endl;
    }
}
